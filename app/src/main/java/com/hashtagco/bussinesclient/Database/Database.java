package com.hashtagco.bussinesclient.Database;
...
public class Database extends SQLiteAssetHelper
{
    ...
    private static final String DATABASE_TABLE ="OrderDetails";
   ...
   /* @Override
    public void onUpgrade(SQLiteDatabase Voedsel, int oldVersion, int newVersion) {
        ...
        String[] sqlSelect = {"UserPhone","ProudactID", "ProudactName", "Quantity", "Discount","Image"};
   ...
   /* public boolean checkFoodExists(String foodId,String UserPhone)
    {
        ...
        Cursor cursor=null;
        String sqlQuery=String.format("SELECT * FROM OrderDetails WHERE ProudactID='%s' " +
                        "and UserPhone='%s'"
                ,foodId,UserPhone);
    ...
    public void addToCarts(Order order)
    {
        ...
        String Query=String.format("INSERT OR REPLACE INTO OrderDetails" +
                        " (UserPhone,ProudactID,ProudactName,Quantity,Discount,Image) VALUES" +
                        " ('%s','%s','%s','%s','%s','%s');",
                                                              order.getUserPhone(),
                                                              order.getProudactID(),
    ...
    public void updateCart(Order order)
    {
        ...
        String query=String.format("UPDATE OrderDetails SET Quantity='%s' WHERE UserPhone='%s' AND ProudactID='%s' "
                ,order.getQuentity(),order.getUserPhone(),order.getProudactID());
    ...
    public void increaseCart(String foodId,String UserPhone)
    {
        ...
        String query=String.format("UPDATE OrderDetails SET Quantity= Quantity+1  WHERE UserPhone='%s'" +
                " AND ProudactID='%s' ",UserPhone,foodId);
    ...
    public void removeFromCart(String proudactID, String phone)
    {
        ...
        String Query=String.format("DELETE FROM OrderDetails WHERE UserPhone='%s' AND ProudactID='%s' ",
                phone,proudactID);
   ...
   /* public void addToFavorites(Favorites food)
    {
        ...
        String query=String.format("INSERT INTO Favorites(foodId,foodName,foodPrice,foodMenuId,foodImage" +
                        ",foodDiscount,foodDescription,userPhone)VALUES('%s','%s','%s','%s','%s','%s','%s','%s');"
